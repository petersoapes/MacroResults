...
for(final = centCount+fociCount+scCount+1; final < roiManager("count")-1; final++){
	roiManager("select", final);
...
//and printing spline and not blobject name
			...
			SC_array_length = Roi.getProperty("SC_array_length");
			wait(500);
			print("this is distal pos  " + distal_array_pos);//these are always empty -- arggh
...
//error with prox_f2C
			 ...
			 + distal_roi_indx + "\t"+  distal_f2C + "\t"  // distal_f2C
			 + ifd + "\t"
			 + rev_status);
			//File.close(f);
			wait(300);			

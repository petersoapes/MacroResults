//code a way to visually mark the centromere to foci orientation
//this could be started at the blob loop
...
function makeBlob(cen){
	...
	roiManager("Select", cen);
	Roi.setProperty("reverse", "no");//maybe this shouldn't be default.
			...
			Roi.getCoordinates(SCx, SCy);
				...
				for(k=0; k < centx.length && !paired; k++){  //k pixels in centromere
						if(Roi.contains(centx[k], centy[k])) { // this test is for if SC runs through centromere
		 						...
		 						roiManager("Select", cen);
//maybe I need to perform mulitple tests (SC pixels 0-10) and set the status based on multiple 								
								if(Roi.contains(SCx[3], SCy[3])) {  //change this to [5], since some ends of SC extend past centromere
...
// , set foci1 to current foci2 values, set foci values as NA		
	...
	if(objclass == "3CO"){
		...
		if(foci1_indx == errfoci){ //3 changed to 1 (is simplest)
			...
			Roi.setProperty("foci3_index", 'NA');//error here
			Roi.setProperty("foci3_array_pos", 'NA');
			roiManager("Rename", roiManager("index")+"_"+"1CO"+"blobject");			
  			roiManager("update");
  			}
